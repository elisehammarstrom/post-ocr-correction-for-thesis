...
def myers_diff(a_lines, b_lines, max_cer=0):
    """
    An implementation of the Myers diff algorithm.

    Arguments:
        a_lines, b_lines: Lists of lines to be compared
        max_cer: Max acceptable error rate for two lines to be considered
            "equal". By passing a max_cer greater than 0, this allows
            "fuzzy" matching of a_lines and b_lines.
    ...
    for d in range(0, a_max + b_max + 1):
        for k in range(-d, d + 1, 2):
            ...
            while (
                x < a_max
                and y < b_max
                and cer(a_lines[one(x + 1)], b_lines[one(y + 1)]) <= max_cer
            ):
                x += 1
                y += 1
